...
# ["#0071b3", "#1e0191", "#369943"]
# ["#22b302","#0c45c2"]
def plot_samples(text,pts,pts_new_list, images=[], fig=None,objs=None, colors = ["#02b322","#0c45c2","#C2450C"],alpha=[0.9,0.9],linewidth=3.0,
    ...
    line, = ax.plot(x_init, y_init, z_init,alpha=0.9,color="red", label="ORIGINAL",linewidth=linewidth) 
    if color_traj:
        norm = matplotlib.colors.Normalize(vmin=-0.5, vmax=0.5)
        fig.colorbar(cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, fraction=0.026, pad=0.04,label='speed change')
    ...
    for i, pts_new in enumerate(pts_new_list):
        ...
        if color_traj:
            ...
            ax.plot(x_new, y_new, z_new,alpha=alpha_, color=color, label=labels[i],linewidth=linewidth) 
    ...
    ax.set_title('\n'.join(w.wrap(text)), fontsize=23, fontname="Times New Roman")
    ...
    if len(images)>0:
        ...
        for i,(obj_name,ax_im, im) in enumerate(zip(objs.keys(),grid, images)):
            # Iterating over the grid returns the Axes.
            ax_im.imshow(im)
            ax_im.axes.xaxis.set_ticklabels([])
            ax_im.axes.yaxis.set_ticklabels([])
            ax_im.set_title(obj_name,fontsize=13)
            # ax_im.set_title(obj_name,fontsize=8)


            ax_im.tick_params(axis='x', colors='white') 
            ax_im.tick_params(axis='y', colors='white') 
    ...
    if show:
        plt.show()
    return fig
def plot_catesian_markers(ax):

    fs=30
    dist = 0.5
    alpha = 0.2
    color = 'grey'
    ax.text(0,dist, 0, "front", 'x',color=color, alpha=alpha, fontsize=fs, ha='center', va='center')
    ax.text(-dist,0, 0, "left", 'y',color=color, alpha=alpha, fontsize=fs, ha='center', va='center')
    ax.text(0,0, -dist, "bottom", 'x',color=color, alpha=alpha, fontsize=fs, ha='center', va='center')


def plot_objs(objs, ax,colors=[]):
    for i,(name,v) in enumerate(objs.items()):
        x,y,z = v["value"]["obj_p"]
        
        color = colors[i] if i < len(colors)-1 else "#"+''.join([random.choice('0123456789') for j in range(6)])    

        ax.scatter(x,y,z, color=color, s=70)
        # print(dir(sc))
        ax.text(x, y, z, name, 'x', color=color, ha='center', fontweight="bold", fontsize=13)
...
def incorporate_speed(wps, dt=0.1, N=50,speed_offset=1):
